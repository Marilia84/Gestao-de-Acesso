...
const Portaria = () => {
  ...
  const [historicoBase, setHistoricoBase] = useState([]); // Histórico vindo da API

  // Estados para o formulário de registro
  const [tipoPessoa, setTipoPessoa] = useState("COLABORADOR"); // COLABORADOR ou VISITANTE
  const [selectedPessoaId, setSelectedPessoaId] = useState(""); // ID da pessoa principal
  const [selectedPortariaId, setSelectedPortariaId] = useState(
    portariasDisponiveis[0]?.id || ""
  );
  const [currentOcupanteSelection, setCurrentOcupanteSelection] = useState(""); // ID do ocupante no select
  const [selectedOcupantes, setSelectedOcupantes] = useState([]); // Array de { id: string, nome: string, identificador: string }

  // Estados para filtros do histórico
  const today = new Date().toISOString().slice(0, 10);
  const [filtroDataDe, setFiltroDataDe] = useState(today); // Data inicial padrão: hoje
  const [filtroDataAte, setFiltroDataAte] = useState(today); // Data final padrão: hoje
  const [filtroTipo, setFiltroTipo] = useState("TODOS"); // TODOS, COLABORADOR, VISITANTE
  const [filtroPortaria, setFiltroPortaria] = useState(""); // ID da portaria ou '' para todas
  ...
  // Busca dados iniciais (colaboradores, visitantes)
  useEffect(() => {
    const fetchInitialData = async () => {
      setLoadingInitial(true);
      setFetchError("");
      try {
        const [colabRes, visitRes] = await Promise.all([
          api.get("/colaboradores"),
          api.get("/visitantes"),
        ]);
        setColaboradores(Array.isArray(colabRes.data) ? colabRes.data : []);
        setVisitantes(Array.isArray(visitRes.data) ? visitRes.data : []);
      } catch (error) {
  ...
  // Busca histórico quando o INTERVALO de datas do filtro muda
  useEffect(() => {
    const fetchHistorico = async () => {
      setLoadingHistorico(true);
      try {
        // Usa os parâmetros 'de' e 'ate'
        const response = await api.get(
          `/acessos/historico?de=${filtroDataDe}&ate=${filtroDataAte}`
        );
        setHistoricoBase(Array.isArray(response.data) ? response.data : []);
      } catch (error) {
        console.error("Erro ao buscar histórico:", error);
        setHistoricoBase([]); // Limpa em caso de erro
      } finally {
        setLoadingHistorico(false);
      }
    };
    // Só busca se ambas as datas estiverem definidas e válidas
    if (
      filtroDataDe &&
      filtroDataAte &&
      ...
      setHistoricoBase([]); // Limpa se as datas forem inválidas ou ausentes
    }
  }, [filtroDataDe, filtroDataAte]); // Depende de ambas as datas

  // Lista para seleção de ocupantes AGORA SÓ TEM COLABORADORES
  const ocupantesDisponiveis = useMemo(() => {
    if (!Array.isArray(colaboradores)) return [];
    // Mapeia colaboradores para o formato necessário
    return colaboradores
      .map((c) => ({
        id: c.idColaborador,
        nome: c.nome,
        identificador: c.matricula, // Identificador é a matrícula
      }))
      .sort((a, b) => a.nome.localeCompare(b.nome)); // Ordena por nome
  }, [colaboradores]);

  // Aplica filtros de tipo e portaria no histórico (front-end)
  ...
  const handleAddOcupante = () => {
    ...
    // Garante que o ocupante não seja a pessoa principal (se a principal for colaborador)
    if (
    ...
    // Busca na lista correta (só colaboradores)
  ...
  const handleRegisterEntry = async (e) => {
    ...
    try {
      await api.post("/acessos/por-matricula", payload);
      alert("Entrada(s) registrada(s) com sucesso!");
      setSelectedPessoaId("");
      setSelectedOcupantes([]);
      setCurrentOcupanteSelection("");
      // Recarrega o histórico com o intervalo ATUAL
      try {
        const histRes = await api.get(
          `/acessos/historico?de=${filtroDataDe}&ate=${filtroDataAte}`
        );
        setHistoricoBase(Array.isArray(histRes.data) ? histRes.data : []);
      } catch (histError) {
  ...
  const handleRegisterExit = async (idAcesso) => {
    ...
    try {
      await api.post(`/acessos/${idAcesso}/saida`);
      alert("Saída registrada com sucesso!");
      // Recarrega histórico com o intervalo ATUAL
      try {
        const histRes = await api.get(
          `/acessos/historico?de=${filtroDataDe}&ate=${filtroDataAte}`
        );
          ...
          {loadingInitial ? (
            ...
            <form onSubmit={handleRegisterEntry}>
              ...
              <div className="mt-6 text-right">
                <button
                  type="submit"
                  className="bg-[#038C4C] text-white py-2 px-6 rounded-md font-semibold hover:bg-[#036f4c] transition-colors"
                >
                  Registrar Entrada(s)
                </button>
              </div>
